...
int main(int argc, char *argv[]) {
  ...
  std::string OUTPUT_REPORT_PATH = "ring_shift_non_blocking.csv";
  if (argc > 4) {
  ...
  g_printf("-----------------\n");
  g_printf("|RingShift(NonBlocking) Session|\n");
  g_printf("-----------------\n");
  ...
  for (int i = 0; i < NUM_EXCHANGES; i++) {
    ...
    MPI_Isend(sub_data, data_size_per_rank, MPI_BYTE, next, 0, MPI_COMM_WORLD, &reqs[0]);
    MPI_Irecv(sub_data, data_size_per_rank, MPI_BYTE, prev, 0, MPI_COMM_WORLD,
             &reqs[1]);
  ...
  g_printf("---------------------\n");
  g_printf("|RingShift(NonBlocking) completed|\n");
  g_printf("---------------------\n");
